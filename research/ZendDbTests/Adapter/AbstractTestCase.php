 ...
 * @version    $Id: AbstractTestCase.php 16562 2009-07-08 18:37:58Z ralph $
 */


/**
 * @see Zend_Db_TestSuite_AbstractTestCase
 */
...
abstract class Zend_Db_Adapter_AbstractTestCase extends Zend_Db_TestSuite_AbstractTestCase
{
    ...
    {
        $clonedUtility = $this->_getClonedUtility(true, array('options' => array(Zend_Db::AUTO_QUOTE_IDENTIFIERS => false)));
    ...
    public function testAdapterDelete()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()->from('zf_products')->order('product_id ASC');
        ...
        $rowsAffected = $this->sharedFixture->dbAdapter->delete('zf_products', "$product_id = 2");
        ...
        $rowsAffected = $this->sharedFixture->dbAdapter->delete('zf_products', "$product_id = 327");
    ...
    public function testAdapterDeleteWhereArray()
    {
        ...
        $ids = $this->sharedFixture->dbAdapter->fetchCol("SELECT $product_id FROM $products ORDER BY $product_id");
        $this->assertEquals(array(1, 2, 3), $ids);

        $rowsAffected = $this->sharedFixture->dbAdapter->delete(
            'zf_products',
    ...
    public function testAdapterDeleteEmptyWhere()
    {
        ...
        $count = $this->sharedFixture->dbAdapter->fetchOne("SELECT COUNT(*) FROM $bugs");
    ...
    {
        $describe = $this->sharedFixture->dbAdapter->describeTable('zf_bugs_view');
    ...
    public function testAdapterFetchAll()
    {
        ...
        $result = $this->sharedFixture->dbAdapter->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1);
    ...
    public function testAdapterFetchAllOverrideFetchMode()
    {
        ...
        $result = $this->sharedFixture->dbAdapter->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1, Zend_Db::FETCH_ASSOC);
        $this->assertEquals(2, count($result));
        ...
        $result = $this->sharedFixture->dbAdapter->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1, Zend_Db::FETCH_BOTH);
        $this->assertEquals(2, count($result));
        $this->assertType('array', $result[0]);
        ...
        $result = $this->sharedFixture->dbAdapter->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", 1);
        $this->assertEquals(2, count($result));
    ...
    public function testAdapterFetchAssoc()
    {
        ...
        $result = $this->sharedFixture->dbAdapter->fetchAssoc("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id DESC", 1);
    ...
    public function testAdapterFetchCol()
    {
        ...
        $result = $this->sharedFixture->dbAdapter->fetchCol("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1);
        $this->assertType('array', $result);
    ...
    public function testAdapterFetchOne()
    {
        ...
        $prod = 'Linux';
        $result = $this->sharedFixture->dbAdapter->fetchOne("SELECT $product_name FROM $products WHERE $product_id > ? ORDER BY $product_id", 1);
    ...
    public function testAdapterFetchPairs()
    {
        ...
        $prod = 'Linux';
        $result = $this->sharedFixture->dbAdapter->fetchPairs("SELECT $product_id, $product_name FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1);
        $this->assertEquals(2, count($result)); // count rows
    ...
    public function testAdapterFetchRow()
    {
        ...
        $result = $this->sharedFixture->dbAdapter->fetchRow("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", 1);
    ...
    public function testAdapterFetchRowOverrideFetchMode()
    {
        ...
        // Test associative array
        $result = $this->sharedFixture->dbAdapter->fetchRow("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", 1, Zend_Db::FETCH_ASSOC);
        $this->assertType('array', $result);
        $this->assertEquals(2, count($result)); // count columns
        $this->assertEquals(2, $result['product_id']);

        // Test numeric and associative array
        $result = $this->sharedFixture->dbAdapter->fetchRow("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", 1, Zend_Db::FETCH_BOTH);
        $this->assertType('array', $result);
    ...
    public function testAdapterInsert()
    {
        ...
        $rowsAffected = $this->sharedFixture->dbAdapter->insert('zf_bugs', $row);
        $this->assertEquals(1, $rowsAffected);
    ...
    public function testAdapterInsertDbExpr()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->fetchOne("SELECT $bug_id FROM $bugs WHERE $bug_id = 5");
    ...
    public function testAdapterLimit()
    {
        ...
        $sql = $this->sharedFixture->dbAdapter->limit("SELECT * FROM $products ORDER BY $product_id", 1);
    ...
    public function testAdapterLimitOffset()
    {
        ...
        $sql = $this->sharedFixture->dbAdapter->limit("SELECT * FROM $products ORDER BY $product_id", 1, 1);
    ...
    {
        try {
            $sql = $this->sharedFixture->dbAdapter->limit('SELECT * FROM zf_products', 0);
        ...
        try {
            $sql = $this->sharedFixture->dbAdapter->limit('SELECT * FROM zf_products', 1, -1);
            $this->fail('Expected to catch Zend_Db_Adapter_Exception');
        } catch (Zend_Exception $e) {
            $this->assertType('Zend_Db_Adapter_Exception', $e,
                'Expecting object of type Zend_Db_Adapter_Exception, got '.get_class($e));
    ...
    {
        $tables = $this->sharedFixture->dbAdapter->listTables();
        $this->assertContains('zf_products', $tables);
    }
    ...
    public function _testAdapterOptionCaseFoldingCommon($case)
    {
        ...
        $clonedUtility = $this->_getClonedUtility(
            true, 
            array('options' => array(Zend_Db::CASE_FOLDING => $case))
            );
        $db = $clonedUtility->getDbAdapter();
        
        $clonedUtility->createDefaultResources();
        $clonedUtility->loadDefaultTableData();
        
        $this->_testAdapterOptionCaseFoldingSetup($db);
        
        
        $productsTable = $db->quoteIdentifier($clonedUtility->getTableName('Products'));
        $productNameColumn = $db->quoteIdentifier('product_name');
        /*
         * it is important not to delimit the pname identifier
         * in the following query
         */
        $sql = "SELECT $productNameColumn AS case_folded_identifier FROM $productsTable";
        $stmt = $db->query($sql);
        $result = $stmt->fetchAll(Zend_Db::FETCH_ASSOC);
        $keys = array_keys($result[0]);
        
        $clonedUtility->cleanupResources();
        unset($clonedUtility, $db);
        return $keys[0];
    }
    ...
    public function testAdapterQueryAfterUnclosedQuery()
    {
        ...
        $stmt1 = $this->sharedFixture->dbAdapter->query($select);
    ...
    {
        try {
            $this->sharedFixture->dbAdapter->query('Bogus query');
            $this->fail('Expected exception not thrown');
        } catch (Zend_Exception $e) {
            $this->assertType('Zend_Db_Statement_Exception', $e,
                'Expecting object of type Zend_Db_Statement_Exception, got '.get_class($e));
    ...
    {
        try {
            $this->sharedFixture->dbAdapter->query('SELECT * FROM BogusTable');
            $this->fail('Expected exception not thrown');
        } catch (Zend_Exception $e) {
            $this->assertType('Zend_Db_Statement_Exception', $e,
                'Expecting object of type Zend_Db_Statement_Exception, got '.get_class($e));
    ...
    {
        $stmt = $this->sharedFixture->dbAdapter->query('SELECT * FROM ' . $this->sharedFixture->dbAdapter->quoteIdentifier('zf_bugs') . ' WHERE '
            . $this->sharedFixture->dbAdapter->quoteIdentifier('bug_id') . ' = -1');
    ...
    public function testAdapterQuoteDbExpr()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->quote($expr);
    ...
    {
        $int = 123;
        $value = $this->sharedFixture->dbAdapter->quote($int);
        $this->assertEquals(123, $value);
    }
    ...
    public function testAdapterQuoteIdentifierDbExpr()
    {
        $string = 'String with "`\' quotes';
        $expr = new Zend_Db_Expr($string);
        $value = $this->sharedFixture->dbAdapter->quoteIdentifier($expr);
        $this->assertEquals($string, $value);
    }
    ...
    public function testAdapterQuoteIntoDbExpr()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->quoteInto($string, $expr);
    ...
    {
        $value = $this->sharedFixture->dbAdapter->quoteInto('foo = ?', 1234, Zend_Db::INT_TYPE);
        $this->assertType('string', $value);
        $this->assertEquals('foo = 1234', $value,
            'Incorrect quoteInto() result for INT_TYPE');

        $value = $this->sharedFixture->dbAdapter->quoteInto('foo = ?', 1234, Zend_Db::BIGINT_TYPE);
        $this->assertType('string', $value);
        $this->assertEquals('foo = 1234', $value,
            'Incorrect quoteInto() result for BIGINT_TYPE');

        $value = $this->sharedFixture->dbAdapter->quoteInto('foo = ?', 12.34, Zend_Db::FLOAT_TYPE);
        $this->assertType('string', $value);
        $this->assertEquals('foo = 12.340000', $value,
            'Incorrect quoteInto() result for FLOAT_TYPE');

        $value = $this->sharedFixture->dbAdapter->quoteInto('foo = ?', 1234, 'CHAR');
        $this->assertType('string', $value);
        $this->assertEquals("foo = 1234", $value,
    ...
    {
        $value = $this->sharedFixture->dbAdapter->quoteInto('foo = ? and bar = ?', 1234, null, 1);
        $this->assertType('string', $value);
    ...
    public function testAdapterQuoteTypeInt()
    {
        foreach ($this->_numericDataTypes as $typeName => $type) {
            ...
            // test int value
            $value = $this->sharedFixture->dbAdapter->quote(1234, $typeName);
            $this->assertType('string', $value);
    ...
    public function testAdapterQuoteTypeBigInt()
    {
        foreach ($this->_numericDataTypes as $typeName => $type) {
            ...
            $value = $this->sharedFixture->dbAdapter->quote('1234e15', $typeName);
            $this->assertType('string', $value);
            $this->assertEquals('1234e15', $value,
                "Incorrect quote() BIGINT_TYPE result for int string");

            $value = $this->sharedFixture->dbAdapter->quote('1234e+15', $typeName);
            $this->assertType('string', $value);
            ...
            // test bigint value
            $value = $this->sharedFixture->dbAdapter->quote('2200000000', $typeName);
            $this->assertType('string', $value);
            ...
            $value = $this->sharedFixture->dbAdapter->quote('020310253000', $typeName);
            $this->assertType('string', $value);
            ...
            $value = $this->sharedFixture->dbAdapter->quote('09091', $typeName);
            $this->assertType('string', $value);
            ...
            $value = $this->sharedFixture->dbAdapter->quote('0x83215600', $typeName);
            $this->assertType('string', $value);
    ...
    public function testAdapterQuoteTypeFloat()
    {
        foreach ($this->_numericDataTypes as $typeName => $type) {
            ...
            $value = $this->sharedFixture->dbAdapter->quote(12.34, $typeName);
            $this->assertType('string', $value);
            ...
            $value = $this->sharedFixture->dbAdapter->quote('12.34abcd', $typeName);
            $this->assertType('string', $value);
            $this->assertEquals('12.34', $value,
                "Incorrect quote() FLOAT_TYPE result");
    ...
    {
        $value = $this->sharedFixture->dbAdapter->quote(1234, 'CHAR');
        ...
        $value = $this->sharedFixture->dbAdapter->quote('1234abcd', 'CHAR');
        $this->assertType('string', $value);
        $this->assertEquals("'1234abcd'", $value,
            "Incorrect quote() CHAR type result");

        $value = $this->sharedFixture->dbAdapter->quote('1234abcd56', 'CHAR');
        $this->assertType('string', $value);
        $this->assertEquals("'1234abcd56'", $value,
            "Incorrect quote() CHAR type result");

        // test Zend_Db_Expr is passed through literally
        $value = $this->sharedFixture->dbAdapter->quote(new Zend_Db_Expr('1+2+3'), 'CHAR');
        $this->assertType('string', $value);
        $this->assertEquals("1+2+3", $value,
    ...
    {
        $value       = ':0\'';
        $valueQuoted = $this->sharedFixture->dbAdapter->quote($value);
        $bugs = $this->sharedFixture->dbAdapter->quoteIdentifier('zf_bugs');
        $bug_id = $this->sharedFixture->dbAdapter->quoteIdentifier('bug_id');
        $bug_status = $this->sharedFixture->dbAdapter->quoteIdentifier('bug_status');
    ...
    public function testAdapterSetFetchMode()
    {
        ...
        foreach ($modes as $mode) {
            $this->sharedFixture->dbAdapter->setFetchMode($mode);
            $this->assertEquals($mode, $this->sharedFixture->dbAdapter->getFetchMode());
        }
    }
    ...
    {
        try {
            $this->sharedFixture->dbAdapter->setFetchMode(-999);
            $this->fail('Expected exception not thrown');
        } catch (Zend_Exception $e) {
            $this->assertType('Zend_Db_Adapter_Exception', $e,
                'Expecting object of type Zend_Db_Adapter_Exception, got '.get_class($e));
    ...
    public function testAdapterUpdate()
    {
        ...
        // an existing row.
        $rowsAffected = $this->sharedFixture->dbAdapter->update(
            'zf_products',
    ...
    public function testAdapterUpdateSetDbExpr()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->fetchOne("SELECT $bug_status FROM $bugs WHERE $bug_id = 1");
    ...
    public function testAdapterUpdateEmptyWhere()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->fetchCol("SELECT $bug_status FROM $bugs");
    ...
    protected function _testAdapterAlternateStatement($stmtClass)
    {
        ...
        $clonedUtility = $this->_getClonedUtility(true, $params);
        $clonedUtility->createDefaultResources()->loadDefaultTableData();
        $db = $clonedUtility->getDbAdapter();
        ...
        $bugs = $this->sharedFixture->dbAdapter->quoteIdentifier('zf2_bugs');
    ...
    public function testAdapterSetCustomProfiler()
    {
        ...
        try {
            $this->sharedFixture->dbAdapter->setProfiler($profilerOptions);
    ...
    {
        $version = $this->sharedFixture->dbAdapter->getServerVersion();
    ...
    public function testAdapterGetConnection()
    {
        ...
        $this->assertTrue($this->sharedFixture->dbAdapter->isConnected());
    }
    ...
    public function testAdapterCloseConnection()
    {
        ...
        $this->assertFalse($dbAdapter->isConnected());
    ...
    public function testAdapterReadClobFetchAll()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->fetchAll("SELECT * FROM $documents WHERE $document_id = 1");
    ...
    public function testAdapterReadClobFetchCol()
    {
        ...
        $value = $this->sharedFixture->dbAdapter->fetchCol("SELECT $document_clob FROM $documents WHERE $document_id = 1");
        $expected = 'this is the clob that never ends...'.
                    'this is the clob that never ends...'.
                    'this is the clob that never ends...';
