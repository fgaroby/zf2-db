 ...
 * @version    $Id: AbstractTestCase.php 16269 2009-06-24 13:38:15Z ralph $
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Zend_Db_Select_AbstractTestCase extends Zend_Db_TestSuite_AbstractTestCase
{
    /**
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', 'product_name'); // scalar
        return $select;
    }
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', array('product_id', 'product_name')); // array
        return $select;
    }
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', array('alias' => 'product_name'));
        return $select;
    }
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', "zf_products.product_name");
        return $select;
    }
    ...
    protected function _selectColumnsExpr()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', new Zend_Db_Expr($products.'.'.$product_name));
        return $select;
    }
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', array('count' => 'COUNT(*)'));
        return $select;
    }
    ...
    protected function _selectDistinctModifier()
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->distinct()
            ->from('zf_products', new Zend_Db_Expr(327));
        return $select;
    }
    ...
    {
        $subquery = $this->sharedFixture->dbAdapter->select()
            ->from('subqueryTable');
    ...
    public function testSelectFromSelectObject()
    {
        ...
        $cmp = 'SELECT ' . $this->sharedFixture->dbAdapter->quoteIdentifier('t') . '.* FROM (SELECT '
                         ...
                         . $this->sharedFixture->dbAdapter->quoteIdentifier('subqueryTable') . ') AS '
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from(array('p' => 'zf_products'), array('product_id', 'product_name'));
        return $select;
    }
    ...
    protected function _selectJoin()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->join('zf_bugs_products', "$products.$product_id = $bugs_products.$product_id");
        return $select;
    }
    ...
    {
        $product_id = $this->sharedFixture->dbAdapter->quoteIdentifier('product_id');
        $xyz1 = $this->sharedFixture->dbAdapter->quoteIdentifier('xyz1');
        $xyz2 = $this->sharedFixture->dbAdapter->quoteIdentifier('xyz2');

        $select = $this->sharedFixture->dbAdapter->select()
            ->from( array('xyz1' => 'zf_products') )
            ->join( array('xyz2' => 'zf_bugs_products'), "$xyz1.$product_id = $xyz2.$product_id")
    ...
    protected function _selectJoinInner()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->joinInner('zf_bugs_products', "$products.$product_id = $bugs_products.$product_id");
        return $select;
    }
    ...
    protected function _selectJoinWithNocolumns()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->join('zf_bugs', "$bugs.$bug_id = 1", array())
            ->join('zf_bugs_products', "$products.$product_id = $bugs_products.$product_id AND $bugs_products.$bug_id = $bugs.$bug_id", null);
        return $select;
    }
    ...
    protected function _selectJoinLeft()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_bugs')
            ->joinLeft('zf_bugs_products', "$bugs.$bug_id = $bugs_products.$bug_id");
        return $select;
    }
    ...
    {
        $bugsBugId        = $this->sharedFixture->dbAdapter->quoteIdentifier('b.bug_id');
        $bugsProductBugId = $this->sharedFixture->dbAdapter->quoteIdentifier('bp.bug_id');

        $select = $this->sharedFixture->dbAdapter->select()
            ->from(array('b' => 'zf_bugs'), array('b.bug_id', 'bp.product_id', 'b.bug_description'))
            ->joinLeft(array('bp' => 'zf_bugs_products'), "$bugsBugId = $bugsProductBugId", array());
    ...
    protected function _selectJoinRight()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_bugs_products')
            ->joinRight('zf_bugs', "$bugs_products.$bug_id = $bugs.$bug_id");
        return $select;
    }
    ...
    protected function _selectJoinQualified()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->join("$schema.zf_bugs_products", "$products.$product_id = $bugs_products.$product_id");
        return $select;
    }
    ...
    protected function _selectJoinUsing()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->joinUsing("zf_bugs_products", "$product_id")
    ...
    protected function _selectJoinInnerUsing()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->joinInnerUsing("zf_bugs_products", "$product_id")
            ->where("$bugs_products.$product_id < ?", 3);
        return $select;
    }
    ...
    public function testSelectJoinCrossUsing()
    {
        ...
        try {
            $select->joinCrossUsing("zf_bugs_products", "$product_id");
            $this->fail('Expected exception of type "Zend_Db_Select_Exception"');
        } catch (Zend_Exception $e) {
            $this->assertType('Zend_Db_Select_Exception', $e,
                              'Expected exception of type "Zend_Db_Select_Exception", got ' . get_class($e));
    ...
    public function testSelectWhereSelectObject()
    {
        ...
        $query = $select->assemble();
        $cmp = 'SELECT ' . $this->sharedFixture->dbAdapter->quoteIdentifier('table') . '.* FROM '
                         . $this->sharedFixture->dbAdapter->quoteIdentifier('table') . ' WHERE (foo IN (SELECT '
                         . $this->sharedFixture->dbAdapter->quoteIdentifier('subqueryTable') . '.* FROM '
    ...
    {
        $price_total = $this->sharedFixture->dbAdapter->quoteIdentifier('price_total');
    ...
    {
        $thecount = $this->sharedFixture->dbAdapter->quoteIdentifier('thecount');

        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_bugs_products', array('bug_id', new Zend_Db_Expr("COUNT(*) AS $thecount")))
            ->group('bug_id')
            ->order('bug_id');
        return $select;
    }
    ...
    protected function _selectGroupByExpr()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_bugs_products', array('bug_id'=>new Zend_Db_Expr("$bug_id+1"), new Zend_Db_Expr("COUNT(*) AS $thecount")))
    ...
    protected function _selectGroupByAutoExpr()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_bugs_products', array('bug_id'=>"ABS($bugs_products.$bug_id)", new Zend_Db_Expr("COUNT(*) AS $thecount")))
    ...
    {
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_bugs_products', array('bug_id', 'COUNT(*) AS thecount'))
            ->group('bug_id')
            ->having('COUNT(*) > 1')
            ->order('bug_id');
        return $select;
    ...
    public function testSelectLimitFetchCol()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products', 'product_name')
            ->where($product_id . ' = ?', 3)
            ->limit(1);

        $result = $this->sharedFixture->dbAdapter->fetchCol($select);
        $this->assertEquals(1, count($result));
    ...
    protected function _selectUnionString()
    {
        ...
        $product_id = $this->sharedFixture->dbAdapter->quoteIdentifier('product_id');
        $product_name = $this->sharedFixture->dbAdapter->quoteIdentifier('product_name');
        $id = $this->sharedFixture->dbAdapter->quoteIdentifier('id');
        $name = $this->sharedFixture->dbAdapter->quoteIdentifier('name');
