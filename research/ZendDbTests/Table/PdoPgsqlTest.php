 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Db_Table_PdoPgsqlTest extends Zend_Db_Table_AbstractTestCase
{
    /*
    ...
    */
    ...
    public function testTableInsert()
    {
        ...
        $lastInsertId = $this->sharedFixture->dbAdapter->lastInsertId('zf_bugs', 'bug_id');
        $lastSequenceId = $this->sharedFixture->dbAdapter->lastSequenceId('zf_bugs_bug_id_seq');
    ...
    public function testTableInsertSequence()
    {
        $table = $this->sharedFixture->tableUtility->getTable('My_ZendDbTable_TableProducts',
            array(Zend_Db_Table_Abstract::SEQUENCE => 'zf_products_product_id_seq'));
        $row = array (
            'product_name' => 'Solaris'
        );
        $insertResult         = $table->insert($row);
        $lastInsertId         = $this->sharedFixture->dbAdapter->lastInsertId('zf_products', 'product_id');
        $lastSequenceId       = $this->sharedFixture->dbAdapter->lastSequenceId('zf_products_product_id_seq');
        $this->assertEquals($insertResult, $lastInsertId);
        $this->assertEquals($insertResult, $lastSequenceId);
    ...
    {
        $tableInfo = $this->sharedFixture->tableUtility->getTableById('Bugs')->info();
